# Awesome LLMs

[简体中文](./README.zh-CN.md) | [Contributing](./CONTRIBUTING.md) | [Shared Docs](./docs/curation-policy.md)

A tree-structured repository for curated LLM-related themes maintained under one Git boundary.

## Structure

This repository is organized as a tree:

- root README: navigation and shared rules
- topic README: topic scope, curation, and reading order
- topic `docs/`: topic-specific notes, primers, maps, and deep dives

## Topic Directories

- [DLLM](./DLLM/README.md) - Diffusion LLM resources, technical notes, and topic-specific reading guides
- [mcp](./mcp/README.md) - Model Context Protocol ecosystem resources, primer, and release checklist
- [skills](./skills/README.md) - LLM and agent skills resources, ecosystem map, and authoring references

## Shared Documents

The following documents are shared across topic directories and live only at the repository root:

- [docs/curation-policy.md](./docs/curation-policy.md)
- [docs/curation-policy.zh-CN.md](./docs/curation-policy.zh-CN.md)
- [docs/link-check.md](./docs/link-check.md)
- [docs/link-check.zh-CN.md](./docs/link-check.zh-CN.md)
- [docs/resource-template.md](./docs/resource-template.md)
- [docs/resource-template.zh-CN.md](./docs/resource-template.zh-CN.md)

## Reading Order

1. Start from this README to choose a topic.
2. Enter the topic README for scope and main entry points.
3. Continue into that topic's `docs/` for detailed topic-specific material.

## Notes

- `CONTRIBUTING.md` and `CONTRIBUTING.zh-CN.md` are shared at the repository root.
- Topic directories keep only topic-specific content plus their bilingual README entry points.
- All Git operations should be understood at the `awesome-LLMs/` repository boundary.
